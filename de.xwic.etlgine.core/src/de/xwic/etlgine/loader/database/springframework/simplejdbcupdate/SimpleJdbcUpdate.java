...
/**
 * A SimpleJdbcUpdate is a multi-threaded, reusable object providing easy update capabilities for a table. It provides meta data processing
 * to simplify the code needed to construct a basic update statement. All you need to provide is the name of the table and a Map containing
 * the column names and the column values.
 *
 * <p>
 * The meta data processing is based on the DatabaseMetaData provided by the JDBC driver. As long as the JBDC driver can provide the names
 * of the columns for a specified table than we can rely on this auto-detection feature. If that is not the case then the column names must
 * be specified explicitly.
 *
 * <p>
 * The actual update is being handled using Spring's {@link org.springframework.jdbc.core.JdbcTemplate}.
 *
 * <p>
 * Many of the configuration methods return the current instance of the SimpleJdbcUpdate to provide the ability to string multiple ones
 * together in a "fluid" interface style.
 *
